...
export async function updateUserProfile(id: string, input: unknown): Promise<ActionResult> {
  const user = await requireUser();
  if (!canManageUsers(user.role)) return { success: false, error: "You don't have permission to manage users." };
  ...
  return safely(async () => {
    ...
    await connectDB();
    const email = parsed.data.email.toLowerCase();
    if (await User.exists({ email, _id: { $ne: id } })) {
      return { success: false, error: "A user with this email already exists." };
    }

    const result = await User.updateOne({ _id: id }, { name: parsed.data.name, email });
    if (result.matchedCount === 0) return { success: false, error: "User not found." };
...
export async function changeUserPassword(id: string, input: unknown): Promise<ActionResult> {
  const user = await requireUser();
  if (!canManageUsers(user.role)) return { success: false, error: "You don't have permission to manage users." };
  ...
  return safely(async () => {
    ...
    await connectDB();
    const passwordHash = await bcrypt.hash(parsed.data.password, 12);
    const result = await User.updateOne({ _id: id }, { passwordHash });
    if (result.matchedCount === 0) return { success: false, error: "User not found." };
